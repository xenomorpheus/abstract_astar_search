 ...
 * Cheating a bit here. Action has a next so they can form a linked list AKA a
 * Path. The data is implementation specific data for an action.
 */
...
/*
 * A Successor is a set containing a Model State, the action that immediately
 * led to that Model State, and the Cost of that single action.
 */
 ...
 */
typedef ai_successor *(*ai_successor_function)(
    ai_model_state *model_state, ai_transition_function transition_function);

// Is Goal State Function returns true if, and only if, the Model State is a
// Goal state.
typedef int (*ai_is_goal_state_function)(ai_model_state *model_state);

// The Goal Estimated Cost Function returns the estimated cost to reach a Goal
// state. This function must be Consistent, that is it may NEVER over-estimate
// the cost.
typedef float (*ai_goal_est_cost_function)(ai_model_state *model_state);

// A function that knows how to duplicate the implementation specific data of a
// Model State.
typedef void *(*ai_model_state_data_duplicator)(void *data);

// A function that knows how to free the implementation specific data of a Model
// State.
typedef void (*ai_model_state_data_free)(void *data);

// A function that knows how to duplicate the implementation specific data of an
// Action.
typedef void *(*ai_action_data_duplicator)(void *data);

// A function that knows how to free the implementation specific data of an
// Action.
