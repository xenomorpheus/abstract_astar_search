...
char *test__ai_path_append_action() {
  ...
  mu_assert(path == action1, "_ai_path_append_action: path start NULL, append "
                             "action. path equals action1.");
  // Add 2nd action
  _ai_path_append_action(&path, action2);
  mu_assert(path == action1, "_ai_path_append_action: path start action1, "
                             "append action2. path equals action1.");
  mu_assert(path->next == action2,
            "_ai_path_append_action: path start action1, append action2. "
            "path's next equals action2.");
  // Add 3rd action
...
void *_my_data_duplicator(void *data) { return (void *)strdup((char *)data); }
...
char *test__ai_path_duplicate() {
  ...
  mu_assert(
      strcmp(new_path->data, old_path->data) == 0,
...
char *test_ai_successor_constructor() {
  ...
  // Test
  mu_assert(successor != NULL, "ai_successor_constructor: successor NOT NULL.");
  ...
  // TODO float	mu_assert(successor->cost == cost, "ai_successor_constructor:
  // cost value.");
...
char *test_ai_fringe_element_constructor() {
  ...
  ai_fringe_element *fe =
      ai_fringe_element_constructor(model_state, path, 1.2f, 3.4f);
...
char *test__ai_fringe_element_list_add_by_total_cost() {
  ...
  ai_fringe_element *node1 =
      ai_fringe_element_constructor(NULL, NULL, 0.0f, 2.0f);
  ai_fringe_element *node2 =
      ai_fringe_element_constructor(NULL, NULL, 0.0f, 1.0f);
  ai_fringe_element *node3 =
      ai_fringe_element_constructor(NULL, NULL, 0.0f, 3.0f);
  mu_assert(list == NULL,
  ...
  mu_assert(list->next == node1, "_ai_fringe_element_list_add_by_total_cost: "
                                 "test2 : list next equals node1.");
  mu_assert(list->next->next == NULL, "_ai_fringe_element_list_add_by_total_"
                                      "cost: test2 : list next next NULL.");
  ...
  mu_assert(list->next == node1, "_ai_fringe_element_list_add_by_total_cost: "
                                 "test3 : list next equals node1.");
  mu_assert(list->next->next == node3,
            "_ai_fringe_element_list_add_by_total_cost: test3 : list next next "
            "equals node3.");
