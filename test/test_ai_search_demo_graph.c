...
// Action specific data. In this implementation a graph of nodes, and moving to one of them.
typedef struct my_action_data_struct {
	char *node_name;
} my_action_data;

// Model State specific data. In this implementation a graph of nodes, and at one of them.
...
my_node_transition s_transition_2 = { .node_name = "B", .cost = 1.f, .next =
NULL, };
my_node_transition s_transition = { .node_name = "A", .cost = 2.f, .next =
		&s_transition_2 };

my_node_transition a_transition_3 = { .node_name = "B", .cost = 1.f, .next =
NULL, };
my_node_transition a_transition_2 = { .node_name = "C", .cost = 3.f, .next =
		&a_transition_3, };
my_node_transition a_transition = { .node_name = "D", .cost = 1.f, .next =
		&a_transition_2 };

my_node_transition b_transition_2 = { .node_name = "G", .cost = 10.f, .next =
NULL, };
my_node_transition b_transition = { .node_name = "D", .cost = 5.f, .next =
		&b_transition_2 };

my_node_transition c_transition =
		{ .node_name = "G", .cost = 7.f, .next = NULL, };

my_node_transition d_transition =
		{ .node_name = "G", .cost = 4.f, .next = NULL, };

my_node_data_set test_transition_set[] = { { .node_name = "S",
		.heuristic = 0.0f, .transition = &s_transition }, { .node_name = "A",
		.heuristic = 3.0f, .transition = &a_transition }, { .node_name = "B",
		.heuristic = 3.0f, .transition = &b_transition }, { .node_name = "C",
		.heuristic = 1.0f, .transition = &c_transition }, { .node_name = "D",
		.heuristic = 2.0f, .transition = &d_transition }, { .node_name = NULL,
		.heuristic = 0.0f, .transition = NULL }, };
...
ai_successor *my_successor_function(ai_model_state *model_state,
	...
	my_node_data_set *node_data = _node_data_lookup(test_transition_set,
			current_node_name);
	if (node_data == NULL) {
		return NULL;
	}
	...
	while (transition) {
		my_action_data *new_action_data = (my_action_data *) malloc(
				sizeof(my_action_data));
		new_action_data->node_name = transition->node_name; // Don't duplicate immutable data.
		ai_action *new_action = ai_action_constructor(new_action_data);
		ai_model_state *new_model_state = transition_function(model_state,
				new_action);
		ai_successor *new_successor = ai_successor_constructor(new_model_state,
				new_action, transition->cost);
		if (successor_head) {
...
void *my_action_data_duplicator(void *data) {
	my_action_data *new_data = (my_action_data *) malloc(
			sizeof(my_action_data));
...
char * test_ai_search_demo_at_goal(void) {
	...
	// Setup
	static my_model_state_data model_state_data = { .node_name = "G", };
...
char * test_ai_search_demo_multi(void) {
	...
	// Setup
    static my_model_state_data model_state_data = { .node_name = "S", };
	ai_model_state *model_state = ai_model_state_constructor(&model_state_data);
	ai_search_astar *astar = ai_search_astar_constructor(&evaluator);
	// Run
	ai_path *path = astar->find_path_to_goal(astar, model_state);
